    ...
    def after_pipeline_run(
        ...
        if self._is_mlflow_enabled:
            if isinstance(pipeline, PipelineML):
                # Avoid duplicate logging within the same MLflow run: if a previous
                # node/group already logged the PipelineML model, skip.
                already_logged = False
                try:
                    if self.run_id is not None:
                        run_info = mlflow.get_run(self.run_id)
                        already_logged = (
                            run_info.data.tags.get("kedro_mlflow_model_logged")
                            == "true"
                        )
                except Exception:
                    # Be permissive: if tag lookup fails, consider not logged
                    already_logged = False

                if already_logged:
                    self._logger.info(
                        "Skipping mlflow model logging: model already logged for run %s",
                        self.run_id,
                    )
                    return

                # When running with --nodes (partial pipeline runs), some artifacts from
                # the training phase may not exist yet. To avoid premature failures, we
                # only attempt to log the PipelineML model when all required artifacts
                # for the inference pipeline are present.
                # Required artifacts are "inference" inputs excluding parameters and the
                # user-provided input dataset.
                required_artifacts = [
                    ds_name
